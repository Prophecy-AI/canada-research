        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "**HARDWARE & TIME CONSTRAINTS (ABSOLUTE):**\n"
                    "• Hardware: 36 vCPUs, 440GB RAM, 1x NVIDIA A10 GPU (24GB VRAM)\n"
                    "• **ABSOLUTE TIME TARGET: 20±10 minutes (10-30 min range) - NON-NEGOTIABLE**\n"
                    "• **30+ min = FAILURE.** Only 40 min allowed for extreme cases (>100GB dataset + no alternatives).\n"
                    "• **EFFICIENCY MANDATE:** Aim for 15-20 min. Faster = better. Speed is a feature.\n"
                    "• **DEFAULT STRATEGY: 2-3 CV folds × 5-8 epochs** = ~12-15 min training + 5 min inference\n"
                    "  - Use 2 folds for large models (EfficientNet-B4+, ViT) - saves 33% time\n"
                    "  - Use 3 folds for small models (ResNet-50, simple NNs)\n"
                    "  - NEVER recommend 5 folds unless absolutely necessary for strategy to work\n"
                    "  - NEVER recommend 10+ epochs unless fast epochs (<1 min each)\n"
                    "• **ESTIMATE TIME BEFORE RECOMMENDING:** Calculate (folds × epochs × min_per_epoch)\n"
                    "  - If estimate >20 min → REJECT strategy, recommend faster approach\n"
                    "  - Examples: 3 folds × 8 epochs × 0.5 min = 12 min ✓ | 3 folds × 8 epochs × 3 min = 72 min ✗\n"
                    "• **MONITORING MANDATE:** If agent reports \"fold 1 took 10+ min\" → tell them to KILL and use 1-2 folds\n"
