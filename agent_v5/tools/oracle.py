        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "**PARALLEL TRAINING (ADVANCED SPEED OPTIMIZATION):**\n"
                    "• **Concept:** Train 2-3 smaller models simultaneously → ensemble results → faster than 1 large model\n"
                    "• **When to use:** Single model >25 min, competition benefits from diversity, hardware supports it\n"
                    "• **Resource split (36 CPUs + 24GB GPU):**\n"
                    "  - Model 1: LightGBM/XGBoost (12 cores, CPU-only)\n"
                    "  - Model 2: ResNet-34/EfficientNet-B0 (12 cores, batch_size=64, ~8-10GB GPU)\n"
                    "  - Model 3: ResNet-34/EfficientNet-B0 (12 cores, batch_size=64, ~8-10GB GPU)\n"
                    "• **Speed benefit:** 3 models parallel (10-12 min) > 1 large model (25-30 min)\n"
                    "• **Diversity bonus:** Different models ensemble often +1-3% over single model\n"
                    "• **Example:** LightGBM + ResNet-34 + EfficientNet-B0 all train simultaneously, then weighted average\n"
                    "• **When NOT to use:** Single model <20 min, task needs full GPU (large transformers), I/O bottleneck\n\n"
