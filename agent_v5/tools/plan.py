...
class PlanTaskTool(BaseTool):
    ...
    Supports both Anthropic (Claude) and OpenAI (o1/o3) models.
    """
    ...
    def __init__(self, workspace_dir: str, get_conversation_history_fn=None):
        ...
        # Supported models:
        #   Anthropic: claude-opus-4-20250514, claude-sonnet-4-5-20250929
        ...
        if self.planning_model.startswith("claude-"):
            from anthropic import Anthropic
            self.anthropic_client = Anthropic(api_key=os.environ["ANTHROPIC_API_KEY"])
        elif self.planning_model.startswith("o") or self.planning_model.startswith("gpt-"):
            from openai import OpenAI
            self.openai_client = OpenAI(api_key=os.environ.get("OPENAI_API_KEY"))
        else:
            raise ValueError(f"Unsupported planning model: {self.planning_model}")
...
"""
        ...
        try:
            # Call appropriate reasoning model for planning
            if self.anthropic_client:
                # Use Anthropic Claude for planning
                ...
                # Note: o1/o3 models don't support system messages or temperature
                response = self.openai_client.chat.completions.create(
                    ...
                    # but we set them for compatibility with other models
                    max_completion_tokens=8000 if self.planning_model.startswith("o") else None,
                    max_tokens=None if self.planning_model.startswith("o") else 8000,
                )
